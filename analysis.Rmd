...
stex  <- fread("stex_with_uriel_distances.csv")
toefl <- fread("toefl_with_uriel_distances.csv")
...
make_group_vfolds <- function(groups, v = 10, repeats = 5, seed = 1234) {
  ...
  for (r in seq_len(repeats)) {
    ...
    split_id <- cut(seq_along(ug_shuf), breaks = v, labels = FALSE)
...
evaluate_glmnet_cv <- function(data, outcome, specs, folds, family = c("gaussian", "poisson")) {
  ...
  for (spec in specs) {
    ...
    if (spec$type == "baseline_mean") {
      fold_res <- rbindlist(lapply(folds, function(fd) {
        y_train <- y[fd$train]
        y_test  <- y[fd$test]
      ...
      all_res[[ctr]] <- fold_res
      ctr <- ctr + 1L
      next
    }

    X <- make_sparse_x(data, spec$rhs)

    fold_res <- rbindlist(lapply(folds, function(fd) {
      x_train <- X[fd$train, , drop = FALSE]
      x_test  <- X[fd$test,  , drop = FALSE]
      y_train <- y[fd$train]
      y_test  <- y[fd$test]

      inner_folds <- min(10, max(3, floor(length(y_train) / 10)))
...
# 4. Data preparation
# ----------------------------
# factorise character covariates where appropriate
stex[, `:=`(
  Sex = factor(Sex),
  C = factor(C),
...
# standardise URIEL distances within each dataset
standardize_cols(stex, distance_vars)
standardize_cols(toefl, distance_vars)

# main STEX analysis set: only actual bilingual pairs with complete distance data
stex_main <- copy(
  stex[
    !is.na(L2_code) &
    L2 != "Monolingual"
...
# pair-level aggregated sensitivity set
stex_pair <- stex_main[, .(
  n = .N,
...
# 5b. STEX pair-level correlations for main outcomes
...
# 7. Inferential: STEX
# ----------------------------
# baseline covariates for inferential models
...
# Count outcomes: Negative binomial
...
# Secondary Gaussian outcomes, if enough data
...
# ----------------------------
# 8. Pair-level STEX sensitivity analysis
# ----------------------------
...
# ----------------------------
# 10. Predictive analysis: STEX
# ----------------------------
# Predictive baselines:
# - baseline includes learner/context variables and categorical context
# - URIEL added incrementally
...
run_stex_predictive <- function(dat, outcome, family = c("gaussian", "poisson")) {
  ...
  # common complete-case data across all candidate predictors
  union_rhs <- paste(
    stex_baseline_rhs,
...
# ----------------------------
# 11. Compact summary tables for reporting
# ----------------------------
# TOEFL: best single-distance term per outcome (by p-value among one-distance models)
...
# STEX: best single-distance term per primary outcome
...
# Predictive: best model per outcome / scheme
